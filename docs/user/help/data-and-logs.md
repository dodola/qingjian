...
- **Linux**：「~/.local/share/qingjian/」（或 `$XDG_DATA_HOME/qingjian/`）；配置文件位于「~/.config/qingjian/」，需自行创建。

Linux 预览版目前只写学习数据，输入日志、统计与词汇表待后续版本接入。
...
测试版缺省开启（Linux 预览版尚未记录）。每次上屏在本机记录一行：按键、切分、看到的第一页候选、所选序号、翻了几页、从第一键到上屏用了多久、所在应用的标识
...
- **Linux**：随 Fcitx5 输出；设置 `QINGJIAN_LOG_DIR` 后按天写入该目录。
